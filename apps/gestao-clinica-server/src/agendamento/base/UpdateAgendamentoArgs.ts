/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AgendamentoWhereUniqueInput } from "./AgendamentoWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { AgendamentoUpdateInput } from "./AgendamentoUpdateInput";

@ArgsType()
class UpdateAgendamentoArgs {
  @ApiProperty({
    required: true,
    type: () => AgendamentoWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AgendamentoWhereUniqueInput)
  @Field(() => AgendamentoWhereUniqueInput, { nullable: false })
  where!: AgendamentoWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => AgendamentoUpdateInput,
  })
  @ValidateNested()
  @Type(() => AgendamentoUpdateInput)
  @Field(() => AgendamentoUpdateInput, { nullable: false })
  data!: AgendamentoUpdateInput;
}

export { UpdateAgendamentoArgs as UpdateAgendamentoArgs };
